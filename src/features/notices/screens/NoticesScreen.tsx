...
const TABS = ['Notices', 'Announcements', 'Notifications'] as const;
...
export const NoticesScreen: React.FC = () => {
  ...
  const { notices, announcements, notifications, readNotice } = useNotices();
  ...
  const renderNotices = () => (
    <FlatList
      ...
      renderItem={({ item }) => (
        <NoticeCard notice={item} onPress={() => readNotice(item.id)} />
      )}
    ...
    <FlatList
      ...
      renderItem={({ item }: { item: AppNotification }) => (
        <View style={[styles.notifItem, !item.isRead && styles.notifUnread]}>
          <Text style={styles.notifIcon}>{TYPE_ICON_MAP[item.type] ?? '🔔'}</Text>
...
const styles = StyleSheet.create({
  ...
  annCard: { backgroundColor: colors.white, borderRadius: 12, padding: 14, marginBottom: 10, elevation: 2 },
  ...
  notifItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.white, borderRadius: 10, padding: 12, marginBottom: 8, elevation: 1 },
